...
export class MsgService {
  ...
  async allMsgEnvoye(userId: number, expediteurId: number) {
    ...
    var ret = await queryRunner.manager.query(
      'SELECT * FROM Msg LEFT JOIN Locataire ON Msg.destinataireId = Locataire.locataireId WHERE Msg.expediteurId = ' +
        expediteurId,
    );
    return { data: ret };
  }
  async allMsgRecu(userId: number, destinataireId: number) {
    ...
    const queryRunner = await appDataSource.createQueryRunner();
    var ret = await queryRunner.manager.query(
      'SELECT * FROM Msg LEFT JOIN Locataire ON Msg.expediteurId = Locataire.locataireId WHERE Msg.destinataireId = ' +
        destinataireId,
    );
    return { data: ret };
  }
  ...
  async allMsgEnvoyeLocatire(userId: number, expediteurId: number) {
    ...
    const queryRunner = await appDataSource.createQueryRunner();
    var ret = await queryRunner.manager.query(
      'SELECT * FROM Msg LEFT JOIN Bailleur ON Msg.destinataireId = Bailleur.bailleurId WHERE expediteurId = ' +
        expediteurId,
    );
    return { data: ret };
  }
  async allMsgRecuLocatire(userId: number, destinataireId: number) {
    ...
    const queryRunner = await appDataSource.createQueryRunner();
    var ret = await queryRunner.manager.query(
      'SELECT * FROM `Msg` LEFT JOIN Bailleur ON Msg.expediteurId = Bailleur.bailleurId WHERE `destinataireId` = ' +
        destinataireId,
    );
    return { data: ret };
  }
  ...
  async sauveMsg(msgData: MsgEntity) {
    try {
      //Ajout à la base
  ...
  async marquLuMsg(userId: number, luMsgDto: LuMsgDto) {
    ...
    const ret = await this.msgRepository.update(
      { msgId },
      {
        msgLu,
      },
